...
import { getProductsForTechnology, technologies } from "@/data/technologies";
...
export default function TechnologyCards() {
  return (
    <div className="mx-auto grid max-w-6xl grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3 md:gap-6">
      {technologies.map((technology) => {
        const relatedProducts = getProductsForTechnology(technology.slug);
        ...
        return (
          ...
          >
            <div className="flex h-14 items-center justify-center text-black">
              <TechnologyIcon
                slug={technology.slug}
                className="h-12 w-12 opacity-90 transition group-hover:scale-105"
              />
            </div>
            <div className="mt-3 h-1 w-12 rounded-full bg-[#8b1010] transition group-hover:w-16" />
